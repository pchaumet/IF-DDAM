...
c     faisceau gaussien de d=-N/2*dx -xs(1) et pareil en y.
      ...
      subroutine gaussiansurfcirc(eps,zcouche,neps,nepsmax,k0,waist,xx0
     $     ,yy0,zz0,thetat,phit,ss,E0,xs,ys,zs,FF0,aretecube,nx,ny,nz
     ...
     $     ,zs(nmax),thetat,phit,k0,aretecube
      double complex eps(0:nepsmax+1),FF0(3*nmax),E0
      integer nfft2d2,nkx,nky,k,ii,jj,kk,indice,kkk,nnn,test,i
      double precision kx,ky,kz,kxx,kyy,kzz,deltak,euler(3,3),kzt
     $     ,eulerinv(3,3),kmax,kp2,theta,phi,pi,const,const1,var1 ,var2
     $     ,xx0,yy0,zz0,x0,y0,z0,fac,fluxinc,fluxref,fluxtrans,irra,ss,s
...
c     polarisation s
                  ...
                  Ays=kzz*const*const1*icomp*s
                  Azs=-kyy*const*const1*icomp*s
c                  Ays=kzz*const*const1
c                  Azs=-kyy*const*const1
