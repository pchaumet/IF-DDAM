...
c     Ici on travaille avec e(k||)
      ...
      subroutine champtotalcirc(imax,ncote,nfft2d,nfft2dmax,nepsmax
     $     ,neps,epscouche,k0,aretecube,NA,Ediffkzpos,Ediffkzneg
     $     ,zcouche,E0,ss,theta,phi,Egausxref ,Egausyref ,Egauszref
...
!$OMP DO SCHEDULE(STATIC) REDUCTION(+:fluxinc) COLLAPSE(2)
     ...
     $           then
               ...
               if (i.eq.imini.and.j.eq.jmini) then
                  fluxinc=fluxinc+(cdabs(E0/deltakx/deltaky)**2)*kz
               endif
            endif
         enddo
      enddo
