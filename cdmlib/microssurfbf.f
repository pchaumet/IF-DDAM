      subroutine microssurfbf(xi,xr,nbsphere,ndipole,nx,ny,nz,nx2,ny2
...
!$OMP END PARALLEL
 ...
 222     deltak=deltakx*dnint(deltakm/deltakx)/dble(2**k)
      ...
      write(*,*) 'Number of incidence',ii,ideltam
...
!$OMP DO SCHEDULE(STATIC)
                  ...
                  do i=1,nbsphere           
                     call champlineairekxky(epscouche,zcouche,neps
     $                    ,nepsmax,xs(i),ys(i),zs(i),k0,E0,ss,pp,kxinc
     $                    ,kyinc ,infostr,nstop,FF0(3*i-2),FF0(3*i-1)
     $                    ,FF0(3*i))
                  enddo
!$OMP ENDDO 
!$OMP END PARALLEL
...
c     calcul champ local
     ...
     $                       ,matindplan,Tabdip,b31,b32 ,b33,FF,FF0
     $                       ,FFloc,b11,b12,b13,a11,a12,a13,a22,a23,a31
     $                       ,a32 ,a33 ,WRK,nlar,ldabi,polarisa
     $                       ,methodeit,tol,tol1 ,nloop ,ncompte ,planf
     $                       ,planb,nstop ,infostr)
                        if (nstop.eq.1) return
                     else
                        call inverserigsurfopt(xi,xr,nbsphere,ndipole,nx
     $                       ,ny,nz,nx2,ny2,nxm,nym,nzm,nplanm,ntotalm
     $                       ,nmax,matindplan,b31,b32 ,b33,FF,FF0,FFloc
     $                       ,b11,b12,b13,a11,a12,a13,a22,a23,a31 ,a32
     $                       ,a33 ,WRK,nlar,ldabi,polarisa,methodeit,tol
     $                       ,tol1 ,nloop,ncompte ,planf,planb,nstop
     $                       ,infostr)
                        if (nstop.eq.1) return
                     endif
...
c     calcul champ dessus
     ...
     $                             *ky.gt.0.d0) then
                                 z=1.d0
                                 kz=dsqrt(k02*indicen*indicen-kx*kx-ky
     $                                *ky)
                                 Emx=0.d0
                                 Emy=0.d0
                                 Emz=0.d0
     ...
     $                                ,Stenseur)
                                 ...
                                 Emx=(Stenseur(1,1)*FF(1)+Stenseur(1,2)
     $                                *FF(2)+Stenseur(1,3)*FF(3))*ctmp
                                 Emy=(Stenseur(2,1)*FF(1)+Stenseur(2,2)
     $                                *FF(2)+Stenseur(2,3)*FF(3))*ctmp
                                 Emz=(Stenseur(3,1)*FF(1)+Stenseur(3,2)
     $                                *FF(2)+Stenseur(3,3)*FF(3))*ctmp
                                 do k=2,nbsphere
                                    kk=3*(k-1)
                                    if (zs(k).ne.zs(k-1)) then
                                       call tenseurmulticoucheloinfft(kx
     $                                      ,ky,kz,z,zs(k),k0,nepsmax
     $                                      ,neps,dcouche,zcouche
     $                                      ,epscouche,Stenseur) 
                                    endif
                                    ctmp=cdexp(-icomp*(kx*xs(k)+ky
     $                                   *ys(k)))
                                    Emx=Emx+(Stenseur(1,1)*FF(kk+1)
     $                                   +Stenseur(1,2)*FF(kk+2)
     $                                   +Stenseur(1 ,3)*FF(kk+3))*ctmp
                                    Emy=Emy+(Stenseur(2,1)*FF(kk+1)
     $                                   +Stenseur(2,2)*FF(kk+2)
     $                                   +Stenseur(2 ,3)*FF(kk+3))*ctmp
...
c     calcul champ dessous
                           if (ncote.eq.-1.or.ncote.eq.0) then
                              if (k02*indice0*indice0*numaperref
     $                             *numaperref*0.9999d0-kx*kx-ky
     $                             *ky.gt.0.d0) then    
                                 z=-1.d0
                                 kz=-dsqrt(k02*indice0*indice0-kx*kx-ky
     $                                *ky)
                                 call  tenseurmulticoucheloinfft(kx,ky
     $                                ,kz,z,zs(1),k0,nepsmax,neps
     $                                ,dcouche,zcouche,epscouche
...
c     passe le champ diffracte lointain en amplitude e(k||)
     ...
     $                 ,epscouche,nepsmax,neps ,numaperref,numapertra,k0
     $                 ,deltax ,deltakx,imaxk0 ,nfft2dtmp,nfft2d,ncote
     $                 ,nstop ,infostr)
                  if (nstop.eq.1) return
                  tmp=indice0*k0
...
!$OMP DO SCHEDULE(STATIC)  COLLAPSE(2)              
                     do i=-imaxk0,imaxk0
                        do j=-imaxk0,imaxk0                         
                           ...
                           if (indice0*indice0*k02*numaperref
     $                          *numaperref*0.9999d0 -kx*kx-ky
     $                          *ky.gt.0.d0) then
     ...
     $                                ,pp,kx,ky,infostr ,nstop ,Arx,Ary
     $                                ,Arz,Atx,Aty ,Atz)
...
!$OMP DO SCHEDULE(STATIC)  COLLAPSE(2)                           
                     do i=-imaxk0,imaxk0
                        do j=-imaxk0,imaxk0
                           ...
                           if (indicen*indicen*k02*numapertra
     $                          *numapertra*0.9999d0-kx*kx-ky
     $                          *ky.gt.0.d0) then
...
!$OMP DO SCHEDULE(STATIC)    
     ...
     $                       *dconjg(Efourierxpos(indice))
     $                       +Eimagexpos(indice)
                        Eimageypos(indice)=Efourierypos(indice)
     $                       *dconjg(Efourierypos(indice))
     $                       +Eimageypos(indice)
                        Eimagezpos(indice)=Efourierzpos(indice)
     $                       *dconjg(Efourierzpos(indice))
     $                       +Eimagezpos(indice)
...
!$OMP DO SCHEDULE(STATIC)    
     ...
     $                       *dconjg(Efourierxneg(indice))
     $                       +Eimagexneg(indice)
                        Eimageyneg(indice)=Efourieryneg(indice)
     $                       *dconjg(Efourieryneg(indice))
     $                       +Eimageyneg(indice)
                        Eimagezneg(indice)=Efourierzneg(indice)
     $                       *dconjg(Efourierzneg(indice))
     $                       +Eimagezneg(indice)
...
!$OMP END PARALLEL
      ...
      if (nmatf.eq.0) then
         ...
         if (ncote.eq.0.or.ncote.eq.1) then
            ...
            open(301,file='imagebfpos.mat')
            open(302,file='imagebfxpos.mat')
            open(303,file='imagebfypos.mat')
            open(304,file='imagebfzpos.mat')
            open(305,file='imageincbfpos.mat')
            open(306,file='imageincbfxpos.mat')
            open(307,file='imageincbfypos.mat')
            open(308,file='imageincbfzpos.mat')
     ...
     $              +Eimageincypos(i)**2.d0+Eimageinczpos(i)**2.d0))
         ...
         if (ncote.eq.0.or.ncote.eq.-1) then
            ...
            open(301,file='imagebfneg.mat')
            open(302,file='imagebfxneg.mat')
            open(303,file='imagebfyneg.mat')
            open(304,file='imagebfzneg.mat')
            open(305,file='imageincbfneg.mat')
            open(306,file='imageincbfxneg.mat')
            open(307,file='imageincbfyneg.mat')
            open(308,file='imageincbfzneg.mat')
