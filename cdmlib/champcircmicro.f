      subroutine champcircmicro(eps,disz,neps,nepsmax,x,y,k0,E0,ss
     $     ,thetat,phit,infostr,nstop,Arx,Ary,Arz,Atx,Aty,Atz)
...
c     on eclaire par en dessous.
      ...
      parameter (nmax=40+2)
      double precision disz(0:nepsmax),x,y,z,phi,theta,phit,thetat ,k0
     $     ,k0d,pi,s,ss,us
      double precision kx,ky,kp2,k02,ka
      double complex eps(0:nepsmax+1),E0,E0s,E0p,E0x,E0y,Exx,Eyy,E0z,Arx
     $     ,Ary,Arz,Atx,Aty,Atz,icomp,uncomp,Esca0,Eprod0,Esca(nmax)
...
c     tet si milieu homogene
 ...
 900     job=11
         ...
         Exx=Esca(1)/dsqrt(2.d0)
         Eyy=Esca(1)*icomp*dsign(us,s)
         Arx=Exx*dcos(phi)-Eyy*dsin(phi)
         Ary=Eyy*dcos(phi)+Exx*dsin(phi)
         Arz=0.d0

         Exx=Esca(max)/dsqrt(2.d0)
         Eyy=Esca(max)*icomp*dsign(us,s)
