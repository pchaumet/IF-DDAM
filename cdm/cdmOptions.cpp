...
Options::Options()
{
  ...
  nfft2dList = (QStringList() << "128" << "256" <<  "512"  << "768" << "1024" << "1536" << "2048" << "3072" << "4096" << "8192" << "16384" << "24576" << "32768" << "49152" << "65536");
...
void Options::initOptions(){ 
  ...
  epsilon11.fill(dcmplx(1,0),MAX_OBJECT_NUMBER);
  for (int i = 0 ; i < MAX_OBJECT_NUMBER; i++)
  ...
  epsilon22.fill(dcmplx(1,0),MAX_OBJECT_NUMBER);
  ...
  epsilon33.fill(dcmplx(1,0),MAX_OBJECT_NUMBER);
  ...
  E0m.fill(dcmplx(1,0),MAX_WAVEMULTI_NUMBER);
  interfacenumber = 1;
  epsilonlayer.resize(MAX_LAYER_NUMBER);
  epsilonlayer.fill(dcmplx(1,0),MAX_LAYER_NUMBER);
  positioninterface.resize(MAX_LAYER_NUMBER);
  positioninterface.fill(0,MAX_LAYER_NUMBER);
  materiallayer.resize(MAX_LAYER_NUMBER);
  materiallayer.fill("xx",MAX_LAYER_NUMBER);
  wavemultinumber = 1;
  nxm = nym = nzm = 10;
  ntheta = 36;
  nphi = 72;
...
void Options::initDb() {
  ...
  query.exec("insert into options_tbl values ('new','',datetime('now'),632.8,1,6328,'Linear plane wave',1,1,'sphere',1,'iso','xx',10,1e-4)");
  QLOG_DEBUG () << query.lastError().text();
  ...
  query.exec("create table media_tbl "
	     "(parent varchar(255) not null, "
             "layernum integer not null, "
	     "epsilonr double, "
	     "epsiloni double, "
	     "position double, "
	     "materiallayer varchar(255), "
             "primary key (parent, layernum))");
  for (int i = 0; i < MAX_LAYER_NUMBER; i++)
    query.exec("insert into media_tbl values ('new'," + QString::number(i) + ",1,0,0,'xx')");
...
Options::saveDb(QString name, QString description){
  ...
  QString queryStr = "insert or replace into options_tbl values ('"+ 
	...
	description + "', datetime('now'), " +
        ...
        this->getBeam() + "', '" +
        QString::number(this->getInterfaceNumber()) +  "', '" +
  ...
  for (int i = 0 ; i < this->getInterfaceNumber()+1; i++) {
   QString queryStr = "insert or replace into media_tbl values ('"+ 
	name + "', " +
        QString::number(i) + ", " +
        QString::number(real(this->getEpsilonlayer().at(i))) +  ", " +
	QString::number(imag(this->getEpsilonlayer().at(i))) +  ", " +
        QString::number(this->getPositioninterface().at(i)) +  ", '" +
        this->getMaterialInterface()[i] + "')";
   query.exec(queryStr);
   QLOG_DEBUG () << "MEDIA_TBL query " << queryStr;
   QLOG_DEBUG () << "MEDIA_TBL " << query.lastError().text();
  }
  for (int i = 0 ; i < this->getObjectNumber(); i++) {
...
for (int i = 0 ; i < this->getWaveMultiNumber(); i++) {
  ...
  queryStr = "insert or replace into run_tbl values ('"+ 
	...
	QString::number(this->getOpticaltorquedensity()) +  ", " +
        ...
        QString::number(this->getNinterp()) +  ", " +
...
Options::loadDb(QString name){
  ...
  if ( name == "" ) name = "new";
  QSqlQuery query(QSqlDatabase::database(dbpath));
  query.exec("select name,description,wavelength,P0,W0,beam,interfacenumber,wavemultinumber,object,objectnumber,"
  ...
  while (query.next()) {
    ...
    this->setInterfaceNumber(query.value(6).toInt());
    this->setWaveMultiNumber(query.value(7).toInt());
    this->setObject(query.value(8).toString());
    this->setObjectNumber(query.value(9).toInt());
    this->setAnisotropy(query.value(10).toString());
    this->setDiscretization(query.value(11).toInt());
  ...
  for (int i = 0 ; i < this->getInterfaceNumber()+1; i++) {
    QString queryStr = "select epsilonr,epsiloni,position,materiallayer "
	       "from media_tbl where parent='" + name + "' "
               "and layernum=" + QString::number(i);
    QLOG_DEBUG() << "Options::loadDb>> MEDIA_TBL query :" <<  queryStr ;
    query.exec(queryStr);
    QLOG_DEBUG () << "SELECT MEEDIA_TBL" << query.lastError().text();
    while (query.next()) {
      QLOG_DEBUG () << "SELECT MMEDIA_TBL" << query.value(0) << " " << query.value(1);
      QLOG_DEBUG () << "SELECT MMEDIA_TBL" << query.value(2) << " " << query.value(3);
      epsilonlayer.replace(i,dcmplx(query.value(0).toDouble(),query.value(1).toDouble()));
      positioninterface.replace(i,query.value(2).toDouble());
      materiallayer.replace(i,query.value(3).toString());
    }
  }
  for (int i = 0 ; i < this->getWaveMultiNumber(); i++) {
...
void 
Options::setInterfaceNumber(int _interfacenumber) {
  interfacenumber = _interfacenumber;
}
void 
Options::setEpsilonlayer(QVector<dcmplx> _epsilonlayer) {
  epsilonlayer = _epsilonlayer;
}
void 
Options::setPositioninterface(QVector<double> _positioninterface) {
  positioninterface = _positioninterface;
}
void 
Options::setMaterialInterface(QVector<QString> _materiallayer) {
for (int i = 0 ; i < interfacenumber+1; i++)
    materiallayer.replace(i,_materiallayer.at(i));
}
void 
...
void 
Options::setNinterp(int _ninterp){
  ninterp = _ninterp;
}
void 
...
void 
Options::setZlensr( double _zlensr){
  zlensr = _zlensr;
}
void 
Options::setZlenst( double _zlenst){
  zlenst = _zlenst;
}
void 
...
int 
Options::getInterfaceNumber(){
  return interfacenumber;
}
QVector<dcmplx>
Options::getEpsilonlayer() {
  return epsilonlayer;
}
QVector<double>
Options::getPositioninterface() {
  return positioninterface;
}
QVector<QString> 
Options::getMaterialInterface(){
