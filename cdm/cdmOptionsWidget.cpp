...
OptionsWidget::OptionsWidget(QMainWindow *_mainwindow, Options *_options)
{
  ...
  interfacenumberLabel = new QLabel("Number of interfaces");
  interfacenumber      = new QSpinBox();
  interfacenumber->setRange(1,MAX_LAYER_NUMBER-1);
  interfacenumber->setFixedWidth(120);
  
  interfaceButton = new QPushButton("Props");
  interfaceButton->setFixedWidth(70);
  connect(interfaceButton, SIGNAL(clicked()), this, SLOT(configureInterface()));
  ...
  ninterpLabel = new QLabel("Green functions");
  ninterp = new QComboBox();
  ninterp->addItems(options->ninterpList);
  ninterp->setCurrentIndex(options->getNinterp());
  ninterp->setFixedWidth(120);
  ...
  zlensrLabel = new QLabel("Position focal plane reflexion (nm):");

  zlenst = new QLineEdit(QString::number(options->getZlenst()));
  ...
  QBoxLayout   *interfacelayout = new QBoxLayout(QBoxLayout::LeftToRight);
  interfacelayout->addWidget(interfacenumber);
  interfacelayout->addWidget(interfaceButton);
  ...
  QFrame *hsepmediaprop0 = new QFrame(this);
  QFrame *hsepmediaprop1 = new QFrame(this);
  hsepmediaprop0->setFrameShape(QFrame::HLine);
  hsepmediaprop0->setFrameShadow(QFrame::Sunken);
  hsepmediaprop1->setFrameShape(QFrame::HLine);
  hsepmediaprop1->setFrameShadow(QFrame::Sunken);
  ...
  layout->addRow(interfacenumberLabel,interfacelayout);
...
subtitle5->setFont(QFont( "Helvetica", 12, QFont::Bold, TRUE ) );
  ...
  // init now interfacenumber spinbox index (triggers the handle)
  QLOG_DEBUG ( ) << "Number of interfaces : " << options->getInterfaceNumber();
  interfacenumber->setValue(options->getInterfaceNumber());
...
OptionsWidget::handleBeamSelectionChanged(int index){
 ...
 if ( beam->currentText() != "Multiple wave" )  {
...
int 
OptionsWidget::getInterfaceNumber(){
  return interfacenumber->value();
}
int 
...
void 
OptionsWidget::setInterfaceNumber(int _interfacenumber){
  interfacenumber->setValue(_interfacenumber);
}
void 
...
void OptionsWidget::configureInterface() {
   this->updateOptions();
   if (interfaceconfigdlg) delete interfaceconfigdlg;
   interfaceconfigdlg = new InterfaceConfigDialog(this,Qt::Widget,options);
   interfaceconfigdlg->exec();
}
...
OptionsWidget::update() {
  ...
  //  nnym->setText(QString::number(options->getNym()));
...
OptionsWidget::updateOptions() {
  ...
  options->setInterfaceNumber(this->getInterfaceNumber());
  ...
  options->setNinterp(ninterp->currentIndex());
  ...
  options->setZlenst(zlenst->text().toDouble());  
  ...
  QLOG_DEBUG() << "OptionsWidget::updateOptions> nenergie " << nenergie->isChecked();
