...
void cdmlibwrapper(Options *options, Run *run, QString *infoMessage, int *stopFlag) 
{
    ...
    // definition multicouche
    
    double zcouche[MAX_LAYER_NUMBER-1];
    dcmplx epscouche[MAX_LAYER_NUMBER];
    char materiaucouche[MAX_LAYER_NUMBER][64];
    int neps = options->getInterfaceNumber()-1;
    char materiallayer[MAX_LAYER_NUMBER][64];
    
    for (int i = 0 ; i < options->getInterfaceNumber()+1; i++)
     epscouche[i] = options->getEpsilonlayer().at(i);
    for (int i = 0 ; i < options->getInterfaceNumber(); i++) 
     zcouche[i] = options->getPositioninterface().at(i);
    for (int i = 0 ; i < options->getInterfaceNumber()+1; i++) {
      for( int j = 0; j < 64; j++ ) {
	materiallayer[i][j] = ' ';
	strncpy(materiallayer[i],(char*)options->getMaterialInterface().at(i).toStdString().c_str(),
	       options->getMaterialInterface().at(i).size());
       QLOG_DEBUG () << "Material layer(" << i << ":" << options->getMaterialInterface().at(i);
      }
    }
  ...
  if (options->getObject() == "cuboid (meshsize)" || options->getObject() == "random spheres (meshsize)" || options->getObject() == "inhomogeneous cuboid (meshsize)") {
   ...
   if (options->getObject() == "arbitrary") {
    ...
    int nmatim =options->getNxm()*options->getNym()-min(options->getNxm(),options->getNym())*(min(options->getNxm(),options->getNym())-1)/2;
    int nbs = nmatim*options->getNzm()*options->getNzm();
    QLOG_DEBUG() << "nplanm:" << nplanm  << "n1m:" << n1m << "nmatim:" << nmatim << "nbs:" << nbs;   
    ...
    double zlensr;
    zlensr = options->getZlensr();
    double zlenst;
    zlenst = options->getZlenst();
    int ntypemic;
   ...
   if (advancedinterfaceCheck == 0 && dipolepsilon == false) {
     if (quickdiffractCheck == 1 || microscopyFFTCheck == 1 || nenergieCheck == 1) {
       ...
       //       if (options->getObject() == "cuboid (meshsize)" || options->getObject() == "random spheres (meshsize)" || options->getObject() == "inhomogeneous cuboid (meshsize)") {
       if (options->getObject() == "cuboid (meshsize)" ) {
	 ...
	 nfftadin = 10*wavelength/side*nxm;
       ...
       if ( options->getObject() == "cuboid (length)" ) {
   ...
   dcmplx *eimagexneg, *eimageyneg, *eimagezneg;
   dcmplx *efourierxneg, *efourieryneg, *efourierzneg;
...
//****************************************************
//   int   n1m,nmatim,nplanm,nbs;
   int  *matindice, *matindplan, *matind;
   dcmplx  *matrange;
   ...
   int needed_mem = run->allocateVectorsMemory(nxm*nym*nzm, ntheta, nphi, nfft2d, objectnumber, n1m, nplanm, nmatim, nbs, nxm, nym, nzm);
   ...
   eimagexneg = run->getEimageXneg();
   eimageyneg = run->getEimageYneg();
   eimagezneg = run->getEimageZneg();
   efourierxneg = run->getEfourierXneg();
   efourieryneg = run->getEfourierYneg();
   efourierzneg = run->getEfourierZneg();
   efourierincxneg = run->getEfourierincXneg();
   efourierincyneg = run->getEfourierincYneg();
   efourierinczneg = run->getEfourierincZneg();
   eimageincxneg = run->getEimageincXneg();
   eimageincyneg = run->getEimageincYneg();
   eimageinczneg = run->getEimageincZneg();
   masque = run->getMasque();
   FF = run->getFF();
   FF0 = run->getFF0();
   FFloc = run->getFFloc();
   xr = run->getxr();
   xi = run->getxi();
   wrk = run->getwrk();
   matindice = run->getmatindice();
   matindplan = run->getmatindplan();
   matind = run->getmatind();
   matrange = run->getmatrange();
   a11 = run->geta11();
   a12 = run->geta12();
   a13 = run->geta13();
   a22 = run->geta22();
   ...
   cdmlibsurf_(&wavelength, beam, object, anisotropy, material,
	   ...
	   &neps, zcouche, (dcmplx*)epscouche, materiallayer,
	       // champ local
	       &localfieldCheck, &macroscopicfieldCheck, &nside, &crosssectionCheck,
	   ...
	   &microscopyCheck, &microscopyFFTCheck, &nenergieCheck, &nobjet,
           ...
           &objectnumber, rayon, xg, yg, zg, (dcmplx*)eps, (dcmplx*)epsani, &lc, &hc, &seed,
	   // ellipsoid
	   ...
	   (dcmplx*)efourierincx, (dcmplx*)efourierincy, (dcmplx*)efourierincz, (dcmplx*)eimagexneg, (dcmplx*)eimageyneg,
...
//     tableaux utilises que dans cdmlib
//****************************************************
//     dimension tableau et tableau d'entier
	   &n1m, &nmatim, &nplanm, &nbs,
	   (dcmplx*)matrange, matindice, matindplan, matind,
...
RunWidget::displayResults()
{
   ...
   if (options->getOpticalforce()) {
	...
	QLabel(QString::number(run->getOpticalForceModulus()),this);
      outputlayout->addRow("Optical force modulus [N/m]:", opticalforcedensity);
   }
   ...
   farfieldComboBox->addItems((QStringList() << "Fourier plane: Scattered field: kz>0" <<  "Fourier plane: Total field: kz>0"
			       ...
			       << "Fourier plane: Scattered field: kz<0" <<  "Fourier plane: Total field: kz<0"
			       << "Image plane: Scattered field: z<0" << "Image plane: Total field: z<0"));
   if (nside == 0) 
   farfieldComboBox->addItems((QStringList() <<  "Fourier plane: Scattered field: kz<0" <<  "Fourier plane: Total field: kz<0"
			       << "Image plane: Scattered field: z<0" << "Image plane: Total field: z<0"));
   ...
   farfieldComboBox->addItems((QStringList() << "Fourier plane: Scattered field: kz>0" <<  "Fourier plane: Total field: kz>0"
     ...
     if (nside == 1) 
       farfieldComboBox->addItems((QStringList() << "Image plane: Scattered field: z>0" << "Image plane: Total field: z>0"
			       << "Image plane: Scattered field: z<0" << "Image plane: Total field: z<0"));
     if (nside == 0) 
       farfieldComboBox->addItems((QStringList() << "Image plane: Scattered field: z<0" << "Image plane: Total field: z<0"));
     if (nside == 2) 
...
RunWidget::plotmicroscopy() {
   ...
   if ( type == "Intensity" ) {
     if ( field == "Image plane: Scattered field: z>0" ) {
     ...
     else if ( field == "Image plane: Total field: z>0" ) {
     ...
     else if ( field == "Image plane: Scattered field: z<0" ) {
       title = "Image plane: Scattered field: z<0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
	  data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], (
                          norm(run->getEimageXneg()[i]) +
                          norm(run->getEimageYneg()[i]) +
                          norm(run->getEimageZneg()[i]))));
          col++;
          if ( col == nfft2d ) {
             col = 0;
             line++;
	  ...
	  data->push_back(QwtPoint3D(run->getKXY()[line],run->getKXY()[col], (
              ...
              norm(run->getEfourierYneg()[i]) +
              norm(run->getEfourierZneg()[i]))));
          col++;
          if ( col == nfft2d ) {
             col = 0;
             line++;
	  ...
	  data->push_back(QwtPoint3D(run->getKXY()[line],run->getKXY()[col], (
              ...
              norm(run->getEfourierincYneg()[i]) +
              norm(run->getEfourierincZneg()[i]))));
          col++;
          if ( col == nfft2d ) {
             col = 0;
             line++;
       ...
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
	  data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], (
                            norm(run->getEimageincXneg()[i]) +
                            norm(run->getEimageincYneg()[i]) +
                            norm(run->getEimageincZneg()[i]))));
          col++;
          if ( col == nfft2d ) {
             col = 0;
             line++;
     ...
     if ( field == "Image plane: Scattered field: z>0" && type == "x" ) {
       title = "Image plane: Scattered field: z>0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageXneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageXneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageincXneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageincXneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
     ...
     else if ( field == "Image plane: Scattered field: z>0" && type == "y" ) {
       title = "Image plane: Scattered field: z>0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Image plane: Total field: z>0" && type == "y" ) {
       title = "Image plane: Total field: z>0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageYneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageYneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageincYneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageincYneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
     ...
     else if ( field == "Image plane: Scattered field: z>0" && type == "z" ) {
       title = "Image plane: Scattered field: z>0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Image plane: Total field: z>0" && type == "z" ) {
       title = "Image plane: Total field: z>0";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageZneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageZneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
       ...
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
         data->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], abs(run->getEimageincZneg()[i])));
         datapc->push_back(QwtPoint3D(run->getXY()[line],run->getXY()[col], arg(run->getEimageincZneg()[i])));
         col++;
         if ( col == nfft2d ) {
            col = 0;
            line++;
...
RunWidget::plotx(QString field, double xvalue) {
    QLOG_DEBUG() << "RunWidget::plotxx> " << field << " " << QString::number(xvalue);
    plotAxislist(field,
	run->getYc(),run->getZc(),run->getXc(),
	run->getYcWF(),run->getZcWF(),run->getXcWF(),
