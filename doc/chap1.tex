...
``script shell''. Suivant les paquets installés on peut alors lancer
./comp (sans FFTW et sans HDF5) ou ./compfftw (sans HDF5) ou
./compfftwhdf5. Attention suivant la configuration de votre ordinateur
peut être lancer avant le code un ``ulimit -s unlimited'' pour ne pas
avoir de problème avec la ``stacksize''. Il se crée donc quatre
exécutables dans chaun des directories test, correspondant chacun à
une configuration. SI on veut changer une donnée il convient bien sûr
d'éditer mainsurf.f et de changer les options directement dans le code
fortran.
...
A noter que le code est donné sans garanti
